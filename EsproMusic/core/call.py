...
async def _get_group_call(self, assistant, chat_id: int):
    """
    Dialog se current group call ka InputGroupCall banata hai.[2][4]
    """
    try:
        dialog = await assistant.get_dialog(chat_id)
        if not dialog or not dialog.chat or not getattr(dialog.chat, "call", None):
            return None
        group_call = dialog.chat.call  # pyrogram.types.GroupCall[2]
...
async def _fetch_vc_participants(self, assistant, chat_id: int) -> set[int]:
    """
    phone.getGroupParticipants se current VC participants ka set(user_id) laata hai.[3][1]
    """
    try:
        ...
        for p in resp.participants:
            ...
            # PeerChannel / PeerChat ko ignore kar rahe hain
...
async def _vc_watch_loop(self, chat_id: int):
    ...
    raw phone.GetGroupParticipants se VC join/leave track karta hai.[1][3]
    """
...
async def speedup_stream(self, chat_id: int, file_path, speed, playing):
    ...
    dur = int(dur)
    played, con_seconds = speed_converter(playing["played"], speed)
    ...
    if str(db[chat_id]["file"]) == str(file_path):
        await assistant.change_stream(chat_id, stream)
    else:
        raise AssistantErr("Umm")
    if str(db[chat_id]["file"]) == str(file_path):
        exis = (playing).get("old_dur")
        if not exis:
            db[chat_id]["old_dur"] = db[chat_id]["dur"]
            db[chat_id]["old_second"] = db[chat_id]["seconds"]
        db[chat_id]["played"] = con_seconds
        db[chat_id]["dur"] = duration
        db[chat_id]["seconds"] = dur
        db[chat_id]["speed_path"] = out
        db[chat_id]["speed"] = speed
...
async def change_stream(self, client, chat_id):
    ...
    try:
        ...
        language = await get_lang(chat_id)
        _ = get_string(language)
        title = (check[chat_id]["title"]).title()
        user = check["by"]
        original_chat_id = check["chat_id"]
        streamtype = check["streamtype"]
        videoid = check["vidid"]
        db[chat_id]["played"] = 0
        exis = (check).get("old_dur")
        if exis:
            db[chat_id]["dur"] = exis
            db[chat_id]["seconds"] = check["old_second"]
            db[chat_id]["speed_path"] = None
            db[chat_id]["speed"] = 1.0
        ...
        if "live_" in queued:
            ...
            db[chat_id]["mystic"] = run
            db[chat_id]["markup"] = "tg"
        elif "vid_" in queued:
            ...
            if videoid == "telegram":
                ...
                run = await app.send_photo(
                    ...
                    caption=_["stream_1"].format(
                        config.SUPPORT_CHAT, title[:23], check["dur"], user
                    ),
                    reply_markup=InlineKeyboardMarkup(button),
                )
                db[chat_id]["mystic"] = run
